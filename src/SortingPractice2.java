...
public class SortingPractice2 {
    ...
    static int[] iterativeMergeSort(int[] array) {
        ...
        // partition an array first into subarrays of size 1
        // group them into pairs
        // merge them into sorted arrays, copy those arrays back to the input array, and repeat the process


        // Initializing the length of the array, n
        int n = array.length;


        // Setting restrictions for the size
        // must be a power of 2
        // must start at 1, creating ann array with a minimum of 2 elements
        for (int size = 1; size <= n/2; size= size*2) {

            // mid variable helps int [] right determine where to start
            int mid= array.length/2;

            // partition it into 1-element segments
            // partition the array again, into pairs of 2-element arrays until its fully sorted

            for (int i = 0; i < array.length;  i= i+2 ) {
                int[] left = slice(array, i, i + size);
                int[] right = slice(array,  size - mid +i, i +2 * size);


                // Using method slice to pass element arrays to method merge
                //slicing 2 elements at a time
                merge(slice(left, 0, 1), slice(right,  1, 2));
                merge(slice(left, 2, 3), slice(right,  3, 4));
                merge(slice(left, 4, 5), slice(right,  5, 6));
                merge(slice(left, 6, 7), slice(right,  7, 8));

                //Doubling the slice length until the slice length is the same size as the original array
                merge(slice(left,0, 2), slice(right,  2, 4));
                merge(slice(left, 4, 6), slice(right,  6, 8));
            }
        }
