...
public class WeirdAlphaHotel {
     ...
     * @return true if there are empty rooms in the hotel.
     */
    ...
    public String remove(int room) {
        ...
        if (this.usage > 0 && this.hotel[room] != null) {
    ...
    public int whichRoom(String guestNane) {
        int room = -1;
        int expectedRoom = this.hash(guestNane);
        int probe = 0;
        while (room == -1 && probe < this.probingLength) {
            int checkRoom = (expectedRoom+probe)%this.numberOfRooms;
            if (this.getGuestName(checkRoom).equals(guestNane)) {
                room = checkRoom;
            }
            probe++;
        }
    ...
    public boolean isGuestHere(String guestName) {
        return this.whichRoom(guestName) != -1;
    }
