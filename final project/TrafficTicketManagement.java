...
public class TrafficTicketManagement {
    ...
    /**
     * Looks up plate and reports findings
     */
    public static void searchForPlate() {
        ...
        //Checking if there is any record of the driver
        if (vehicle == null){
            System.out.printf("\nThere is no record for license plate number %s\n", licensePlateNumber);
        } else {
            //Reports all information found from the plate
            System.out.printf("\n%s The following records were found for plate number %s %s", licensePlateNumber,
                    vehicle.color, vehicle.make, vehicle.model, vehicle.make);
            // Get for more information on the plate
            // search for ticket violations under the plate
            int ticketCounter = 0;
            for (Map.Entry<Integer, TrafficTicket> trafficTicketEntry: trafficTickets.entrySet()){
                // Get the value V of the <K,V> pair stored in the trafficTickets hashmap;
                TrafficTicket  trafficTicket = trafficTicketEntry.getValue();
